...
> referința.
>
> **Dată:** 2026-09-02. **Public:** backend devs și devops pe proiectul următor.
>
> Ordonat după cât timp economisește următorului om, nu pe subsisteme.

---

## Partea 1 — Pentru dezvoltatori

### 1.1 Predarea prin semaforul `.FLG` ⭐ *cea mai portabilă idee de aici*
...
Un extract scris parțial nu poate fi niciodată procesat pe jumătate. Fără lacăte, fără
coordonare, fără o bază de date comună între două echipe care nu se apelează niciodată.
Funcționează pe orice object store. **Folosiți asta pentru orice predare de fișiere între
echipe.**

### 1.2 Ecuația de echilibru ca poartă de build
...
Apoi: **recon iese cu cod nenul și pică DAG-ul.** „O rulare care nu se echilibrează este o
...
### 1.3 Contracte ca date, impuse la build
...
pe classpath-ul Java, iar o **regulă `maven-enforcer` pică build-ul dacă lipsește**, deci un
jar nu poate fi livrat vreodată fără el. Python citește același fișier. `ArtefactsTest`
folosește manifestul de pe disc ca oracol, deci rescrierea unui nume direct în Java pică
build-ul.

Așa oprești două limbaje să divergă. (Avertisment: `ARCHITECTURE.md`, slăbiciunea #1, notează
că asta acoperă *numele*, nu *tipurile* — un IDL adevărat ar fi mai bun. Luați tiparul,
cunoscându-i plafonul.)

### 1.4 Nu puneți niciodată o valoare implicită pe un câmp de identitate
...
### 1.5 O scriere asincronă nelivrată este un eșec, nu un succes lent
...
### 1.6 Citire mărginită dintr-un flux nemărginit
...
### 1.9 Separați nucleul decidabil de I/O

`ReconciliationMatcherTest` testează logica de diferență de mulțimi fără Kafka și fără
BigQuery.

Orice are forma „ia din două locuri, compară, decide" ar trebui să aibă jumătatea de
*comparat și decis* apelabilă cu două colecții în memorie.

### 1.10 REST scris de mână în loc de SDK-uri de furnizor — dar cu o cusătură
...
### 1.11 Comentarii care consemnează incidentul, nu codul
...
Un access token trăiește ~1h; un DAG e de lungă durată. Înghețarea unuia în pod spec e o
credențială care expiră în mijlocul migrării. Aplicațiile care vorbesc HTTP brut nu preiau
automat Workload Identity — deci **luați token-ul de la metadata server-ul GKE și reîmprospătați-l**,
...
imagine de câțiva GB cu `image_pull_policy=Always`. Asta a depășit 600s și a picat cu *„Pod
took longer than 600 seconds to start"* — un timeout de infrastructură deghizat în eșec de
pipeline. Clusterele calde pornesc în sub un minut, deci plafonul mai mare costă timp doar la
...
roluri deliberat înguste (recon are voie doar să *citească* BigQuery). Scrierea de roluri
înguste este irosită dacă fiecare pod rulează tot pe contul comun — **`service_account_name` la
...
VPC, acordări IAM — atârnă de un singur flag, iar un apply dezactivat nu produce nimic și nu
costă nimic.
...
Managed Kafka e intern în VPC. Cloud Run nu îl poate atinge fără un conector Serverless VPC
Access. Asta s-a descoperit *după* ce mock-ul fusese deja livrat și a blocat un criteriu de
acceptanță. **Verificați accesibilitatea la design, nu la testare.**
...
întreabă „a funcționat asta vreodată pe infrastructură reală", răspunsul e o cale, nu o
amintire.
...
### 2.12 Containerizați testul de fum
...
1. **Semaforul `.FLG` scris ultimul** (1.1) — portabil la orice echipă, orice stocare, azi.
2. **Verificați valoarea returnată de fiecare flush asincron** (1.5) — asta a pierdut date în tăcere.
3. **Nu puneți valoare implicită pe un câmp de identitate** (1.4) — și asta a pierdut date în tăcere.
4. **Ecuația de echilibru cu un cod de ieșire** (1.2) — transformă un raport într-o poartă.
